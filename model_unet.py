...
class MaskFeatureAdapter(nn.Module):
    """将 decoder 特征适配为更适合文字区域建模的中间表示。"""
...
class TextRegionGate(nn.Module):
    """根据 mask feature 和 mask probability 对主干特征做区域门控。"""

    def __init__(self, feat_ch, gate_ch):
        super().__init__()
        self.gate_proj = nn.Sequential(
            nn.Conv2d(gate_ch + 1, feat_ch, 1),
    ...
    def forward(self, feat, gate_feat, mask_prob):
        gate_input = torch.cat((gate_feat, mask_prob), dim=1)
        gate = self.gate_proj(gate_input)
        return feat * (1.0 + gate), gate


class StructureDecoderGate(nn.Module):
    """Project structure prior into decoder space and use it to modulate features."""

    def __init__(self, prior_ch, feat_ch):
        super().__init__()
    ...
    def forward(self, feat, structure_prior, gate_scale=1.0):
        if structure_prior.shape[-2:] != feat.shape[-2:]:
            structure_prior = nn.functional.interpolate(
                structure_prior,
                size=feat.shape[-2:],
                mode="bilinear",
                align_corners=False,
            )
        prior_feat = self.adapter(structure_prior)
        gate = self.gate(prior_feat)
        gate_scale = float(max(0.0, gate_scale))
        if gate_scale > 0.0:
            feat = feat + gate_scale * gate * prior_feat
        return feat, gate, prior_feat
...
class SimpleUNet(nn.Module):
    def __init__(
        ...
        self.structure_prior_channels = structure_prior_channels
        self.use_decoder_structure_gate = use_decoder_structure_gate
        self.structure_gate_strength = float(structure_gate_strength)
        self.structure_gate_time_power = float(max(1e-6, structure_gate_time_power))
        ...
        if self.use_structure_prior:
            self.structure_prior_proj = nn.Sequential(
                nn.Conv2d(structure_prior_channels, down_channels[0], 3, padding=1),
                nn.BatchNorm2d(down_channels[0]),
                nn.ReLU(inplace=True),
            )
        else:
            self.structure_prior_proj = None
        ...
        # mask-guided 路线：先提取 mask feature，再预测文字区域概率，并将其回注主干
        self.mask_feature_adapter = MaskFeatureAdapter(up_channels[-1], up_channels[-1])
        ...
        self.text_region_gate = TextRegionGate(up_channels[-1], up_channels[-1])
        if self.use_decoder_structure_gate:
            decoder_gate_channels = tuple(up_channels[1:])
            self.decoder_structure_gates = nn.ModuleList(
                [StructureDecoderGate(structure_prior_channels, ch) for ch in decoder_gate_channels]
            )
        else:
            self.decoder_structure_gates = nn.ModuleList()
    ...
    def forward(self, x, timestep, cond=None, text_emb=None, structure_prior=None):
        """
        ...
        timestep_norm = timestep.float() / 999.0
        low_noise_scale = (1.0 - timestep_norm).clamp(0.0, 1.0).pow(self.structure_gate_time_power)
        ...
        # 3. 初始处理
        x = self.conv0(x)
        if self.use_structure_prior and structure_prior is not None and self.structure_prior_proj is not None:
            if structure_prior.shape[-2:] != x.shape[-2:]:
                structure_prior = nn.functional.interpolate(
                    structure_prior,
                    size=x.shape[-2:],
                    mode="bilinear",
            ...
            x = x + self.structure_prior_proj(structure_prior)
        ...
        for i, up in enumerate(self.ups):
            ...
            x = up(x, t, cond_vec)
            if self.use_decoder_structure_gate and structure_prior is not None and i < len(self.decoder_structure_gates):
                gate_scale = self.structure_gate_strength * low_noise_scale.mean().item()
                x, _, _ = self.decoder_structure_gates[i](x, structure_prior, gate_scale=gate_scale)
        ...
        mask_feature = self.mask_feature_adapter(x)
        mask_pred = self.seg_head(mask_feature)
        x, gate_map = self.text_region_gate(x, mask_feature, mask_pred)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"输出 recon 形状: {outputs['noise_pred'].shape}")
        print(f"输出 mask 形状: {outputs['mask_pred'].shape}")

        out = outputs["noise_pred"]

...
class TestRunAllCLI(unittest.TestCase):
    ...
    def test_full_eval_runs_three_stages(self):
        ...
        normalized_ocr_args = [arg.replace("\\", "/") for arg in ocr_args]
        self.assertIn("eval_outputs/cmp_local/ocr_metrics_detail.csv", normalized_ocr_args)
        self.assertIn("eval_outputs/cmp_local/ocr_metrics_summary.json", normalized_ocr_args)
        self.assertIn("--ocr_summary_json", report_args)
        normalized_report_args = [arg.replace("\\", "/") for arg in report_args]
        self.assertIn("eval_outputs/cmp_local/ocr_metrics_summary.json", normalized_report_args)
        self.assertIn("--report_json", report_args)
        self.assertIn("full_eval_report_summary.json", report_args)

    def test_labels_init_forwards_args(self):
        with patch("run_all.run_python_script", return_value=0) as mocked:
            code = run_all.main([
                "labels-init",
                "--input_dir",
                ...
                "eval_inputs/labels.csv",
                "--manifest_csv",
                "eval_inputs/manifest.csv",
                "--keep_missing",
            ])

        self.assertEqual(code, 0)
        forwarded = mocked.call_args[0][1]
        self.assertIn("--input_dir", forwarded)
        self.assertIn("eval_inputs", forwarded)
        self.assertIn("--output_csv", forwarded)
        self.assertIn("eval_inputs/labels.csv", forwarded)
        self.assertIn("--manifest_csv", forwarded)
        self.assertIn("eval_inputs/manifest.csv", forwarded)
        self.assertIn("--keep_missing", forwarded)

    def test_real_ocr_eval_runs_three_stages_without_gt_dir(self):
        with patch("run_all.run_python_script", side_effect=[0, 0, 0]) as mocked:
            code = run_all.main([
                "real-ocr-eval",
                "--dry_run",
        ...
        self.assertNotIn("--gt_dir", batch_args)
        self.assertIn("--pred_dir", ocr_args)
        normalized_ocr_args = [arg.replace("\\", "/") for arg in ocr_args]
        self.assertIn("eval_outputs/real_eval/diffusion", normalized_ocr_args)
        self.assertIn("eval_outputs/real_eval/ocr_metrics_detail.csv", normalized_ocr_args)
        self.assertIn("eval_outputs/real_eval/ocr_metrics_summary.json", normalized_ocr_args)
        self.assertIn("--ocr_backend", ocr_args)
        self.assertIn("rapidocr", ocr_args)
        self.assertIn("--ocr_summary_json", report_args)
        normalized_report_args = [arg.replace("\\", "/") for arg in report_args]
        self.assertIn("eval_outputs/real_eval/ocr_metrics_summary.json", normalized_report_args)
